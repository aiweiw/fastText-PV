...
void printUsage() {
  std::cerr
    ...
    << "  PVDBOW                  train the paragraph vector with distributed bag of words."
    << std::endl;
}
...
void printPredictPVDBOWUsage() {
    std::cerr
      << "usage: fasttext predictPVDBOW <model> <test-data> <label-save>\n\n"
      << "  <model>      model filename\n"
      << "  <test-data>  test data filename\n"
      << "  <label-save> filename: predicted labels' vectors to be saved\n"
      << std::endl;
...
void predictPVDBOW(const std::vector<std::string>& args) {
  if (args.size() != 5) {
    printPredictPVDBOWUsage();
    exit(EXIT_FAILURE);
  }
  ...
  fasttext.predictPVDBOW(ifs, std::string(args[4]));
  ifs.close();
...
void PVDBOW(const std::vector<std::string> args) {
  std::shared_ptr<Args> a = std::make_shared<Args>();
  a->parseArgs(args);
  FastText fasttext;
  fasttext.train(a);
