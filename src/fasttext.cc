...
void FastText::saveLabelVector() {
  ...
  for (int32_t i = dict_->nwords(); i < dict_->nwords() + dict_->nlabels(); i++) {
    ...
    if (args_->model == model_name::PVDM) {
      ...
      addInputVector(vec, i - dict_->nwords());
    }
    ofs << word << " " << vec << std::endl;
  }
  ofs.close();
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    ...
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (w + c >= 0 && w + c < line.size()) {
        model.predictPVDBOW(labels, line[w + c], lr);
      }
    }
  }
  */

  // Sample a text window, then sample a random word from the text window and
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    int32_t c = uniform(rng);
    std::vector<int>::iterator ele = find(elements.begin(), elements.end(), w + c);
    if (w + c >= 0 && w + c < line.size() && ele == elements.end()) {
      elements.push_back(w + c);
      model.predictPVDBOW(labels, line[w + c], lr);
    }
  }
}

void FastText::predictPVDBOW(
        const std::vector<int32_t>& line,
        std::shared_ptr<Matrix> labelMat,
        real lr) {
  if (line.size() == 0 || labelMat->m_ == 0 || labelMat->n_ == 0) return;
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    ...
    if (w + c >= 0 && w + c < line.size() && ele == elements.end()) {
      elements.push_back(w + c);
      model_->predictPVDBOW(labelMat, line[w + c], lr);
    }
  }
}
...
void FastText::predictPVDBOW(std::istream& in, const std::string& output) {

  tokenCount = 0;
  start = clock();

  std::shared_ptr<Matrix> preLabel = std::make_shared<Matrix>(1, args_->dim);
  preLabel->zero();
  ...
  for (int i = 0; i < args_->epoch; i++) {
    ...
    predictPVDBOW(words, preLabel, lr);
    tokenCount += linetokens;
  ...
  for (int32_t i = 0; i < preLabel->m_; i++) {
    for (int32_t j = 0; j < preLabel->n_; j++) {
      ofs << preLabel->at(i, j) << " ";
    }
    ofs << std::endl;
  }
...
void FastText::trainThread(int32_t threadId) {
  ...
  while (tokenCount < args_->epoch * ntokens) {
    ...
    real lr = args_->lr * (1.0 - progress);
    if (lr < args_->lr * 0.0001) lr = args_->lr * 0.0001; // min learning rate
    if (args_->model == model_name::sup) {
...
void FastText::train(std::shared_ptr<Args> args) {
  ...
  if (args_->model == model_name::PVDBOW) {
    // todo: load pretrained-WordVectors.
    std::cerr << "\rPretrain word vectors" << std::endl;
    args_->model = model_name::sg;
    startThreads();

    std::cerr << "\rPVDBOW train" << std::endl;
    args_->model = model_name::PVDBOW;
    input_ = std::make_shared<Matrix>(dict_->nlabels(), args_->dim);
    input_->zero();
    startThreads();
  } else {
    startThreads();
